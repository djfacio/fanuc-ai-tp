...
Use `tools\New-FanucTaskStateKarelSource.ps1` for the narrow KAREL task-state helper. The generated `A_TSKSTAT` source uses `GET_TSK_INFO` and `SET_INT_REG` only; it must not run, pause, abort, resume, select, or move programs. Treat it as a compile/deploy-tested dependency before using it to clear the `F_FLEXI_LOADER` single-instance blocker.

...
Use `tools\Get-FanucWorkflowMigrationReviewPacket.ps1` to surface concise human decisions from workflow migration specs. Do not expect the operator to review raw JSON, schemas, or validators unless the generator/tooling itself is under review.
Use `tools\New-FanucTaskStateKarelSource.ps1` for the narrow KAREL task-state helper. The generated `TSKSTATUS` source is a reviewed exception to the A_ prefix because it is a reusable utility called as `CALL TSKSTATUS('TASK_NAME',base_register)`. It uses `GET_TPE_PRM`, `GET_TSK_INFO`, and `SET_INT_REG` only; it must not run, pause, abort, resume, select, or move programs. Treat it as a compile/deploy-tested dependency before using it to clear async-task single-instance blockers.
Use `C:\Dev\Fanuc Robot Manuals` as the trusted local FANUC manuals project. For R-30iB Mate Plus references, use `C:\Dev\Fanuc Robot Manuals\R30iB Mate Plus\build-manual-index.ps1` after PDFs are placed there, then `search-manuals.ps1` for focused lookups. Cite manual ID/title and page/section in project notes. Do not copy manuals into this public repo and do not use random public PDF mirrors as authority for generated robot code.

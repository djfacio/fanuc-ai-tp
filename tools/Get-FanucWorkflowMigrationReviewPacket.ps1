...
$lines.Add("- Wait timeout controller write value: $($spec.waitPolicy.controllerWriteValue)")
$lines.Add("- Wait timeout controller storage units: $($spec.waitPolicy.controllerStorageUnits)")
...
$lines.Add("- Use `TSKSTATUS('F_FLEXI_LOADER',91,0)` as the single-instance proof before `RUN F_FLEXI_LOADER`. Current policy: pre-run `204` or `404` may start the task; pre-run `200` is an ownership/state alarm; after `RUN`, only `200` proves success.")
$lines.Add("- Use the 60-second global external wait timeout and explicitly write the timeout variable immediately before each bounded wait.")
